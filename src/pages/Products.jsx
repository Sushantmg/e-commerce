    ...
    return (
        <div className="bg-green-50 dark:bg-gray-900 p-6 transition-colors duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                <h2 className="text-2xl font-semibold text-green-800 dark:text-green-300">Fresh Fruits & Vegetables</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredProducts.length === 0 ? (
                    <p className="text-center text-gray-600 dark:text-gray-400 col-span-full">No products found.</p>
                ) : (
                    filteredProducts.map(product => (
                        <div key={product.id} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 flex flex-col items-center">
                            <img
                                src={product.image}
                                alt={product.title}
                                loading="lazy"
                            ...
                            />
                            <h3 className="text-lg font-bold text-green-700 dark:text-green-300 mb-1">{product.title}</h3>
                            <p className="text-sm text-gray-600 dark:text-gray-300 text-center mb-2">{product.description}</p>
                            <div className="mb-2">
                                {product.discount > 0 ? (
                                    <div className="text-sm text-red-600 dark:text-red-400">
                                        <span className="line-through mr-2">${product.price.toFixed(2)}</span>
                                        <span className="font-bold text-green-800 dark:text-green-200">${product.discountedPrice.toFixed(2)}</span>
                                        <span className="ml-1 text-xs text-red-500 dark:text-red-300">({product.discount}% OFF)</span>
                                    </div>
                                ) : (
                                    <div className="text-green-700 dark:text-green-200 font-semibold text-sm">${product.price.toFixed(2)}</div>
                                )}
                            </div>
                            <button
